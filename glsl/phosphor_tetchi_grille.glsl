...
void main() {
    // [Modified] Decode sRGB from previous pass back to Linear space for accurate math
    vec4 raw_color = COMPAT_TEXTURE(Texture, vTexCoord);
    vec3 center_color = pow(raw_color.rgb, vec3(2.2));
    
    // [Modified] Physically accurate luminance weights for linear space
    float luminance = dot(center_color, vec3(0.2126, 0.7152, 0.0722));

    // [Modified] Reduced bloom strength for linear space (2.5 -> 1.5)
    float bloom_strength = 1.5; 
    float radius = (luminance * bloom_strength) / OutputSize.x; 
    
    vec3 bloom_color = vec3(0.0);
    // [Modified] Convert surrounding pixels to Linear space immediately upon sampling
    ...
    // [Modified] Removed excessive brightness boost and white balance bias (1.3 -> 1.1)
    ...
    float mask_pos = fract((vTexCoord.x * OutputSize.x) / (3.0 * scale));
    vec3 mask = vec3(0.0);
    
    // [Modified] Increased mask base brightness (0.5 -> 0.7) to prevent crushed blacks
    float mask_base = 0.7; 
    ...
    // [Modified] Lowered excessive contrast to prevent over-saturation (1.3 -> 1.1)
    float contrast = 1.1;
    float midpoint = 0.5;    
    ...
    // Final gamma correction for monitor output (Linear -> sRGB)
