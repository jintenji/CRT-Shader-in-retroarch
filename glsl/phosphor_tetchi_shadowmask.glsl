...
void main() {
    ...
    // Adjusted bloom strength for linear space (3.2 -> 1.5)
    float bloom_strength = 1.5; 
    float radius = (luminance * bloom_strength) / OutputSize.x; 
    
    vec3 bloom_color = vec3(0.0);
    // Apply linear conversion to surrounding pixels immediately upon sampling
    ...
    // Normalize brightness boost (1.3 -> 1.1) and neutralize white balance
    float brightness_boost = 1.1; 
    vec3 white_balance = vec3(1.0, 1.0, 1.0); 
    bloom_color *= (brightness_boost * white_balance);

    // Delta/Dot Mask generation
    float mask_scale = MASK_SCALE; 
    vec2 pos = (vTexCoord * OutputSize) / mask_scale;
    float stagger = mod(floor(pos.y), 2.0) * 1.5; 
    
    float mask_pos = fract((pos.x + stagger) / 3.0);
    vec3 mask = vec3(0.0);
    
    // Increase mask base brightness (0.5 -> 0.7) to prevent crushed blacks
    float mask_base = 0.7; 
    ...
    // Remove/reduce secondary mask brightness boost and balance
    float mask_brightness = 1.0; 
    vec3 mask_balance = vec3(1.0, 1.0, 1.0); 
    final_color *= (mask_brightness * mask_balance);

    // Lower contrast to prevent over-saturation (1.3 -> 1.1)
    float contrast = 1.1;
    float midpoint = 0.5;
    ...
    // 8. Re-compress to sRGB for monitor output
